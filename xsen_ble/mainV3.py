    ...
    while True:
        if (not notifyQueue.empty()) and (not lastStateQueue.empty()):
            ...
            if not haveInitYaw:
                focusYaw = curYaw
                haveInitYaw = True
                rightAngle_thread_high = cal_angle(focusYaw, -85)
                rightAngle_thread_low = cal_angle(focusYaw, -90)
                leftAngle_thread_high = cal_angle(focusYaw, 90)
                leftAngle_thread_low = cal_angle(focusYaw, 85)
